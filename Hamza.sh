#!/bin/bash
# Kernel build script for Miatoll - Debug Version

# Stop on errors and trace pipes
set -e
set -o pipefail
...
compile_kernel() {
    ...
    # Auto-discovery for config files
    ...
    if [ -n "$CONF_FILE" ]; then
        ...
        echo "--> Using Config: $FINAL_CONF"
        # Create .config with verbose error reporting
        make -j$(nproc --all) O=$OUT_DIR ARCH=arm64 "$FINAL_CONF" || { echo "ERROR: Kconfig failed"; exit 1; }
    else
        echo "--> ERROR: No miatoll config found!"
        exit 1
    fi

    echo "--> Starting compilation (Verbose Mode)..."
    # Added V=1 for detailed logs to catch where it stops
...
compile_kernel() {
    ...
    # Auto-discovery: Search for miatoll.config or miatoll_defconfig
    # This finds the relative path from arch/arm64/configs/
    CONF_FILE=$(find arch/arm64/configs/ -name "miatoll.config" -o -name "miatoll_defconfig" | head -n 1)
    
    if [ -n "$CONF_FILE" ]; then
        # Strip the prefix to get the config name for make
        FINAL_CONF=$(echo $CONF_FILE | sed 's|arch/arm64/configs/||')
        echo "--> Found Config: $FINAL_CONF"
        make -j$(nproc --all) O=$OUT_DIR ARCH=arm64 "$FINAL_CONF"
    else
        echo "--> ERROR: No miatoll config found in the entire tree!"
...
compile_kernel() {
    ...
    # Direct check and build for found config files
    if [ -f "arch/arm64/configs/miatoll.config" ]; then
        echo "--> Found: miatoll.config in root configs"
        make -j$(nproc --all) O=$OUT_DIR ARCH=arm64 miatoll.config
    elif [ -f "arch/arm64/configs/vendor/Xiaomi/miatoll.config" ]; then
        ...
        echo "--> Error: All config attempts failed!"
        exit 1
    fi
